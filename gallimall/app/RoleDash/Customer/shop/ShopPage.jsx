...
const getImageUrl = (image) => {
  ...
  if (
    ...
    image.includes('res.cloudinary.com/gallimall')
  ) {
    return image;
  }

  // ✅ If it's a full HTTP/HTTPS path from other sources
  if (image.startsWith('http')) return image;

  // ✅ If it's a Cloudinary public ID (e.g., "v1234/filename.jpg")
  if (!image.startsWith('/')) {
    return `https://res.cloudinary.com/gallimall/image/upload/${image}`;
  }

  // ✅ If it's a local Django media path (e.g., "/media/image.jpg")
...
useEffect(() => {
  const fetchShop = async () => {
    if (shopIdQuery && /^\d+$/.test(shopIdQuery)) {
      // ✅ Case 1: shopId in URL
      dispatch(fetchShopDetails(shopIdQuery));
    } else if (navigator.geolocation) {
      // ✅ Case 2: Try location-based shop
      navigator.geolocation.getCurrentPosition(
        async (pos) => {
          const { latitude, longitude } = pos.coords;
          const nearestAction = await dispatch(fetchNearestShopId({ lat: latitude, lon: longitude }));

          if (fetchNearestShopId.fulfilled.match(nearestAction)) {
            const nearestId = nearestAction.payload;

            if (typeof nearestId === 'number') {
              dispatch(fetchShopDetails(nearestId));
            } else {
              console.warn('Nearest shop ID is not valid:', nearestId);
              fetchFallbackShop();
            }
          } else {
            console.warn('No nearest shop found, using fallback.');
            fetchFallbackShop();
          }
        },
        (err) => {
      ...
      // ✅ Case 3: Geolocation not available
      fetchFallbackShop();
    }
  };
  ...
  const fetchFallbackShop = async () => {
    try {
      ...
      if (data.length > 0) {
        dispatch(fetchShopDetails(data[0].id)); // ✅ Load first available shop
      } else {
        console.warn('⚠️ No shops exist in database.');
      }
    } catch (err) {
      console.error('⚠️ Fallback fetch failed:', err);
    }
  };
  ...
  if (!nearestShopId && !shopIdQuery && !loading) {
  return (
    <div className="text-center mt-20 text-gray-600 text-lg">
      🚫 Sorry, no shops are available near your location.
    </div>
  );
}
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {products.map((product) => {
            const quantity = quantities[product.id] || 0;
              const imageUrl = getImageUrl(product.image_url || product.image);  // ✅ updated line
